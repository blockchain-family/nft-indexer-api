...
pub async fn get_nft(
    ...
    Json(param): Json<NFTParam>,
) -> impl IntoResponse {
    let nft = catch_error_500!(s.db.get_nft_details(&param.nft).await);
    ...
    let collection = catch_error_500!(collect_collections(&s.db, &collections_ids).await);

    let mut auction = HashMap::default();
    if let Some(ref auction_id) = nft.auction {
        let a = catch_error_500!(s.db.get_nft_auction(auction_id).await);
        if let Some(a) = a {
            auction.insert(auction_id.clone(), Auction::from_db(&a, &s.db.tokens));
        }
    };

    let mut direct_sell = HashMap::default();
    if let Some(ref direct_sell_id) = nft.forsale {
        let a = catch_error_500!(s.db.get_direct_sell(direct_sell_id).await);
        if let Some(a) = a {
            direct_sell.insert(
                direct_sell_id.clone(),
                DirectSell::from_db(&a, &s.db.tokens),
            );
        }
    };
    ...
    let traits = s.db.get_traits(&nft_addr).await;
...
pub async fn get_nft_direct_buy(
    State(s): State<Arc<HttpState>>,
    Json(params): Json<NFTParam>,
) -> impl IntoResponse {
    let limit = params.limit.unwrap_or(100);
    let offset = params.offset.unwrap_or_default();
    let nft = params.nft;
    let status = params.status.as_deref().unwrap_or_default();
    let list = catch_error_500!(s.db.list_nft_direct_buy(&nft, status, limit, offset).await);
...
pub async fn get_nft_price_history(
    State(s): State<Arc<HttpState>>,
    Json(query): Json<NftPriceHistoryQuery>,
) -> impl IntoResponse {
    ...
    let to = DateTime::from_timestamp(query.to, 0)
        .expect("Failed to get datetime")
        .naive_utc();
    let list = catch_error_500!(s.db.list_nft_price_history(&query.nft, from, to).await);
...
pub async fn get_nft_top_list(
    State(s): State<Arc<HttpState>>,
    Json(params): Json<NFTTopListQuery>,
) -> impl IntoResponse {
    ...
    match cached_value {
        None => {
            let from = DateTime::from_timestamp(params.from, 0)
                ...
                .naive_utc();
            let list =
                catch_error_500!(s.db.nft_top_search(from, params.limit, params.offset).await);
            response = catch_error_500!(make_nfts_response(list, s.db.clone()).await);
            let value_for_cache =
...
pub async fn get_nft_list(
    State(s): State<Arc<HttpState>>,
    Json(params): Json<NFTListQuery>,
) -> impl IntoResponse {
    ...
    match cached_value {
        None => {
            ...
            let list = if search_params.verified {
                catch_error_500!(s.db.nft_search_verified(search_params,).await)
            } else {
                catch_error_500!(s.db.nft_search(search_params,).await)
            };

            let mut r = catch_error_500!(make_nfts_response(list, s.db.clone()).await);
            if !with_count {
...
pub async fn get_nft_random_list(
    State(s): State<Arc<HttpState>>,
    Json(params): Json<NFTListRandomBuyQuery>,
) -> impl IntoResponse {
    let hash = calculate_hash(&params);
    ...
    match cached_value {
        None => {
            ...
            let list = catch_error_500!(s.db.nft_random_buy(max_price, limit).await);
            let mut r = catch_error_500!(make_nfts_response(list, s.db.clone()).await);
...
/// GET /nfts/sell-count
pub async fn get_nft_sell_count(
    State(s): State<Arc<HttpState>>,
    Json(params): Json<NFTSellCountQuery>,
) -> impl IntoResponse {
    let hash = calculate_hash(&params);
    ...
    match cached_value {
        None => {
            let max_price = params.max_price;
            let sell_count =
                catch_error_500!(s.db.nft_sell_count(max_price).await).unwrap_or_default();
...
pub async fn get_nft_for_banner(State(s): State<Arc<HttpState>>) -> impl IntoResponse {
    ...
    match cached_value {
        None => {
            nft_for_banner = catch_error_500!(s.db.nft_get_for_banner().await);
...
pub async fn get_nft_types(
    State(s): State<Arc<HttpState>>,
    Json(params): Json<NFTType>,
) -> impl IntoResponse {
    ...
    match cached_value {
        None => {
            let list_of_types = catch_error_500!(s.db.nft_get_types(verified_flag).await);
...
async fn make_nfts_response(
    list: Vec<NftDetails>,
    db: crate::db::queries::Queries,
) -> anyhow::Result<VecWith<NFT>> {
...
pub async fn collect_nfts(
    db: &crate::db::queries::Queries,
    ids: &[String],
) -> anyhow::Result<HashMap<String, NFT>> {
...
pub async fn get_nfts_price_range(
    State(s): State<Arc<HttpState>>,
    Json(query): Json<NftPriceRangeParams>,
) -> impl IntoResponse {
    let prices = catch_error_500!(
        s.db.nft_price_range_verified(
            &query.collections,
            &query.attributes,
            &query.owners,
...
pub async fn get_my_best_offer(
    State(s): State<Arc<HttpState>>,
    Query(query): Query<MyBestOfferForNftQuery>,
    headers: HeaderMap<HeaderValue>,
) -> impl IntoResponse {
    let user_address = catch_error_401!(s.auth_service.authenticate(headers));

    let best_offer_db_opt = catch_error_500!(
        s.db.get_best_user_offer_for_nft(&user_address, &query.nft)
            .await
    );
    let best_offer_db = catch_empty!(best_offer_db_opt, "offer not found");
    let bets_offer = DirectBuy::from_db(&best_offer_db, &s.db.tokens);
