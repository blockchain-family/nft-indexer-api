...
select c.address                                                                                                 as "collection!",
       ...
       ( select count(*)
         from nft n
         where n.collection = c.address
           and not n.burned )::int                                                                               as "nfts_count!",
       (count(1) over ())::int                                                                                   as "total_rows_count!"
from nft_collection c
         left join lateral ( select min(case when n.address is not null then tup.usd_price * na.min_bid
                                             else 0 end) as price_usd
                             from nft_auction na
                                      join roots r on r.address = na.root
                             ...
                             where na.status = 'active'
                               and (na.finished_at = to_timestamp(0) or na.finished_at > now()) ) as auction on true
         ...
         left join lateral ( select min(case when n.address is not null then tup.usd_price * ds.price else 0 end) as price_usd
                             from nft_direct_sell ds
                                      join roots r on r.address = ds.root
                             ...
                             where ds.state = 'active'
                               and (ds.expired_at = to_timestamp(0) or ds.expired_at > now()::timestamp) ) as direct_sell
                   on true
         ...
         left join lateral ( select sum(case when ag.period_type = 'current' then ag.price_usd else 0 end)  cur,
                             ...
                             from ( select p.period_type,
                                    ...
                                    from periods p
                                             ...
                                             left join roots r on r.address = ndb.root
